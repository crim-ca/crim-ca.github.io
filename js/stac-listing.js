/*
  Find all repositories with GitHub Pages  under the 'crim-ca' organization that correspond to STAC definitions.
  Generates the HTML listing of found pages.
*/
(async () => {
  const response = await fetch(`https://api.github.com/orgs/crim-ca/repos`);
  const data = await response.json();
  pages = data.filter(f => { return f.has_pages && f.description.includes("STAC") })
  let htmlString = `<ol>`;
  for (let p of pages) {
